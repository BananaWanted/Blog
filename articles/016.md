...
<button onclick="load_user_config()">加载配置</button>
...
<button onclick="save_pac()">下载</button>
</div>
...
<script>
    ...
    function update_config(skip_load = false) {
        ...
        var normalize = function(user_input) {
            ...
            var arr = str.split("\n");
            arr.forEach(function(v, i, a) { a[i] = v.trim(); });
            return arr;
        };
    ...
    function save_pac() {
        var pac_blob = new Blob([$pac_content.val()], {type: "text/javascript;charset=utf-8"});
