...
Windows平台个人不怎么熟悉，但是在POSIX世界中，还有一种错误叫做“信号”。在这时，封装的线程最好使用`std::thread::native_handler()`将信号做统一处理。扩展上述的`thread_info`类来记录相关情况。否则进程崩溃什么的都是家常便饭。关于信号的问题，将在以后的博文中详述。
